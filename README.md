...
## Содержание репозитория
  - Папка "3D-models" содержит все модели старой версии робота
  - Папка "New robot models" содержит модели новой версии робота
...
### Камера
  Для определения цвета знаков мы решили использовать модуль технического зрения OpenMV. Он является сенсорным устройством для исследования окружающего пространства путем обработки и анализа изображения со встроенной видеокамеры. Данная камера проста в программировании и всего за пару минут можно полностью написать код поиска кубиков, как это сделали мы.

### Драйвер двигателя
   В качестве драйвера двигателя используется модуль drv8833. Данный модуль неприхотлив, прост в использовании и идеально подходит к нашему двигателю.
...
### Датчики линии QTR-1A 
  Датчики линии QTR-1A - это модуль, имеющий малые габариты и имеющий на борту один датчик отражения.при поможи связки таких датчиков (по два на колесо) и специального черно-белого трафарета на колесе у нашего робота появилась возможность определять пройденный путь задними колесами. Использование данных модулей являлось самы простым вариантом решения задачи подсчета пройденного пути.
### Печатная плата
...
### Фоторгавии системы рулевого управления
  ![Система](https://github.com/Timofei1412/Cabinet-car/blob/main/robot_photo/%D0%A0%D1%83%D0%BB%D0%B5%D0%B2%D0%BE%D0%B5%20%D1%83%D0%BF%D1%80%D0%B0%D0%B2%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5.jpg)
  ![Подшипники]((https://github.com/Timofei1412/Cabinet-car/blob/main/robot_photo/%D0%9F%D0%BE%D0%B4%D1%88%D0%B8%D0%BF%D0%BD%D0%B8%D0%BA%D0%B8.jpg))

  
## Подсистема питания и сенсоры робота 

### Питание
  Источником питания робота является литий-полимерная батарея с номинальным напряжением 7.4 вольта. Для полного отключения питания робота, на плате размещена схема состоящая из кнопки с фиксацией своего положения и p канального транзистора. Это позволяет безопасно комутировать больщие токи при этом используя маломощьную кнопку. На плате установлен делитель напряжения который позволяет контроллеру безопасно считать текущее напряжение акумулятора и предотвратить его разряд. На плате напряжение с акумулятора понижается до 5в при помощи схемы на основе модуля [MP1584](https://alex-exe.ru/radio/power/dc-dc-step-down-mp1584/) для подачи его на лидар серву и камеру, также напряжение 5в дополнительно понижается до 3.3в при помощи линейного стабилизатора для подачи на ESP32 и энкодер. Для подачи на драйвер двигателя напряжение с акумулятора повышается до 10в при помощи схемы на основе модуля [LM2577](https://ampero.ru/lm2577-voltage-regulator-povyshayuschiy-dc-dc-preobrazovatel-3v-34v-v-4v-35v.html). 
