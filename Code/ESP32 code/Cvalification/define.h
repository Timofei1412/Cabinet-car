...
#define tics_per_rotatiton 800
#define mm_per_rotation 132
#define ideal_turn_radius 202
...
extern long long enc_count_global;
...
struct path{
    ...
    void reverce(){
        ...
        for(int i=0; i < this->points_count/2; i++){
            tmp_point = this->points[this->points_count - i - 1];
            this->points[this->points_count - i - 1] = this->points[i];
            this->points[i] = tmp_point;
        }
        for(int i=0; i < this->optimised_points_count/2; i++){
            tmp_point = this->optimised_path[this->optimised_points_count - i - 1];
            this->optimised_path[this->optimised_points_count - i - 1] = this->optimised_path[i];
            this->optimised_path[i] = tmp_point;
        }
        for(int i=0; i < this->points_count; i++){
            this->points[i].y = 3000 - this->points[i].y;
        }
        for(int i=0; i < this->optimised_points_count; i++){
            this->optimised_path[i].y = 3000 - this->optimised_path[i].y;
        }
    }
